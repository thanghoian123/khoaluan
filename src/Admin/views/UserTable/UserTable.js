  ...
  return (
    <TableCell key={key} align={align}>
      ...
      <Tooltip
        ...
        title="Remove"
        placement="top"
        onClick={() => onDelData(item.username)}
      >
...
function UserTable() {
  ...
  useEffect(() => {
    setOpen(true);
    APIs.getListUser(page, itemPerPage, accessToken)
      .then(async (resp) => {
        console.log(resp.data);
        if (resp.data) {
          await dispatch(actions.getUsers(resp.data.dataResponse.list));
          setOpen(false);
        }
      })
  ...
  var onDelData = (userName) => {
    setOpen(true);
    APIs.delUser(userName, accessToken).then((resp) => {
      console.log(resp.data);
      if (resp.data.error == null) {
        ...
        dispatch(actions.delUser(userName));
      } else {
        setOpen(false);
        setOpenDialog({
          ...openDialog,
